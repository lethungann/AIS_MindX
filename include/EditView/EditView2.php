...
class EditView
{
    ...
    function createFocus()
    {
        ...
        if (empty($this->focus->id))
        {
            ...
            //Thêm id vào edit view
            if(isset($_POST['return_module']) && isset($_POST['return_id']) &&
                $_POST['module'] == "a_Receipts" &&
                isset($_POST['return_name']) && $_POST['return_module'] == "a_Invoice") {
                $this->focus->invoice_id = $_POST['return_id'];
                $this->focus->invoice_name = $_POST['return_name'];
            }

            if(isset($_POST['return_module']) && isset($_POST['return_id']) &&
                $_POST['module'] == "a_Contracts" && isset($_POST['return_name'])
                && $_POST['return_module'] == "a_Students") {
                $this->focus->student_id = $_POST['return_id'];
                $this->focus->student_name = $_POST['return_name'];
                $s1 = "SELECT l1.id, l1.name, l1.date_start, l1.date_end, l1.code_class, l1.status 
                FROM a_class l1, a_students l2 
                WHERE l1.deleted = 0 AND l1.id = l2.class_id AND l2.deleted = 0 
                AND l2.id = '{$_POST['return_id']}'";
                $r1 = $GLOBALS['db']->fetchOne($s1);
                if(!empty($r1['id'])) {
                    $newDate = new DateTime($r1['date_end']);
                    $newDate->modify('+7 days');
                    $newDate = $newDate->format('Y-m-d');
                    $this->focus->date_end = $newDate;
                    $html = '<p>'. $mod_strings['LBL_DATA_CLASS_ARRAY']['code_class'] . ': '.$r1['code_class'].'</p>';
                    $html .= '<p>'. $mod_strings['LBL_DATA_CLASS_ARRAY']['name'] . ': '.$r1['name'].'</p>';
                    $html .= '<p>'. $mod_strings['LBL_DATA_CLASS_ARRAY']['enrollment_date'] . ': '.$r1['date_start'].'</p>';
                    $html .= '<p>'. $mod_strings['LBL_DATA_CLASS_ARRAY']['end_date'] . ': '.$r1['date_end'].'</p>';
                    $this->focus->data_class = $html;
                }
            }

            if(isset($_POST['return_module']) && isset($_POST['return_id']) &&
                $_POST['module'] == "a_Budget" &&
                isset($_POST['return_name']) && $_POST['return_module'] == "a_Class") {
                $this->focus->class_id = $_POST['return_id'];
                $this->focus->class_name = $_POST['return_name'];
                $this->focus->user_id = $current_user->id;
                $this->focus->user_name= $current_user->full_name;
            }
        }
    }
