...
class logicBudget
{
    ...
    public function handleAsset(&$bean, $event, $arguments) {
        global $app_list_strings;
        $list_status = $app_list_strings['budget_status_list'];
        if($bean->status == 'ChuaPheDuyet') {
            $bean->status = '<span style="color: #e57842;">' .$list_status['ChuaPheDuyet'].'</span>';
        } elseif($bean->status == 'DaPheDuy') {
            $bean->status = '<span style="color: #33abc7;">' .$list_status['DaPheDuy'].'</span>';
        } elseif($bean->status == 'DangThucHien') {
            $bean->status = '<span style="color: #0c900c;">' .$list_status['DangThucHien'].'</span>';
        }
